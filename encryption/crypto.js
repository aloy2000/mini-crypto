let AES = {
  ...
  cipher: function(input, w) {
    ...
    let state  = [new Array(nb), new Array(nb), new Array(nb), new Array(nb)];
...
AES.Base64 = {
  ...
  decode: function (input) {
    ...
    let chr1, chr2, chr3, dec1, dec2, dec3, dec4;
...
AES.Crypto = function(key) {
  ...
  this.run = function(input) {
    let blockCount = Math.ceil(input.length / blockSize);
    let output     = new Array(input.length);

    let counterBlock, byteCount, offset;
